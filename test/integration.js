...
const parseBuffer = (res, fn) => {
  // Superagent does not detect PDF
  const data = [];
...
describe("integration", () => {
  describe("GET /stats", () => {
    it("should disallow access without key", (done) => {
      request.get("/stats").set("Accept", "application/json").expect(403, done);
    });
      ...
      request
        ...
        .expect((res) => {
          if (res.statusCode !== 200) {
            const errMsg = `Invalid response code: ${
              res.statusCode
          ...
          execSync(
            "curl --upload-file ./example_failed.png https://transfer.sh/example_failed.png",
            { stdio: "inherit" }
          );
          throw new Error(
            `${examplePngPath} does not match rendered screenshot`
          );
        })
        .end(done);
    });
    ...
    it("should render valid pdf from fixtures/example.html", function renderPdf(done) {
      ...
      request
        ...
        .expect((res) => {
          ...
          execSync(
            "curl --upload-file ./example_failed.pdf https://transfer.sh/example_failed.pdf",
            { stdio: "inherit" }
          );
